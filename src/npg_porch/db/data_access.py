...
from npg_porch.models import Pipeline, Task, TaskStateEnum, TaskExpanded, Version
...
class AsyncDbAccessor:
    ...
    async def create_pipeline(self, version: Version) -> Version:
        session = self.session
        assert isinstance(version, Version)
        assert isinstance(version.pipeline, Pipeline)

        pipe = DbPipeline(
            name=version.pipeline.name, repository_uri=version.pipeline.uri
        )
        ver = DbVersion(version=version.version, pipeline=pipe)
        session.add(pipe)
        session.add(ver)
        await session.commit()
        return ver.convert_to_model()

    async def create_version(self, version: Version) -> Version:
        session = self.session
        assert isinstance(version, Version)
        try:
            pipeline = await self._get_pipeline_db_object(version.pipeline.name)
    ...
    async def create_task(self, token_id: int, task: Task) -> tuple[Task, bool]:
        ...
        db_version = await self._get_version_db_object(
            version=task.version.version, name=task.version.pipeline.name
        )
        ...
        try:
            ...
            t = await self.get_db_task(
                pipeline_name=task.version.pipeline.name,
    ...
    async def claim_tasks(
        self, token_id: int, version: Version, claim_limit: int | None = 1
    ) -> list[Task]:
        session = self.session

        try:
            version = await self._get_version_db_object(
                name=version.pipeline.name, version=version.version
            )
        except NoResultFound:
            raise NoResultFound("Pipeline not found")
    ...
    async def update_task(self, token_id: int, task: Task) -> Task:
        ...
        try:
            db_ver = await self._get_version_db_object(
                name=task.version.pipeline.name, version=task.version.version
            )
        except NoResultFound:
            raise NoResultFound("Pipeline not found")
    ...
    async def get_long_running_tasks(self) -> list[TaskExpanded]:
        ...
        for task in tasks:
            ...
            if task.status == TaskStateEnum.DONE:
                lengths.setdefault(task.version.pipeline.name, []).append(
                    (task.updated - task.created).total_seconds()
                )
            else:
                not_done.setdefault(task.version.pipeline.name, []).append(task)
        long_running = []
