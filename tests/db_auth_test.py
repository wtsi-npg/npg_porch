...
async def test_token_string_is_valid(async_minimum):
    ...
    with pytest.raises(
        CredentialsValidationException, match=r"The token should be 32 chars long"
    ):
    ...
    with pytest.raises(
        CredentialsValidationException, match=r"Token failed character validation"
    ):
...
async def test_token_is_known_and_valid(async_minimum):
    ...
    with pytest.raises(
        CredentialsValidationException, match=r"An unknown token is used"
    ):
    ...
    result = await async_minimum.execute(
        select(Token).filter_by(description="OpenStack host, job finder")
    )
    ...
    with pytest.raises(
        CredentialsValidationException, match=r"A revoked token is used"
    ):
